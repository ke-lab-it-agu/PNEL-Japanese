...
class PointerNet(object):
  ...
  def __init__(self, batch_size=128, max_input_sequence_len=5, max_output_sequence_len=7, 
    ...
    # Create placeholders
    self.inputs = tf.placeholder(tf.float32, shape=[self.batch_size,self.max_input_sequence_len,1124], name="inputs")
    ...
    special_token_embedding = tf.get_variable("special_token_embedding", [3,1124], tf.float32, tf.contrib.layers.xavier_initializer())
    # Embedding_table
  ...
  def step(self, session, inputs, enc_input_weights, outputs=None, dec_input_weights=None, update=True):
    ...
    if not self.forward_only and update:
      output_feed = [self.update, self.summary_op, self.loss, self.predicted_ids_with_logits, self.shifted_targets, self.debug_var, self.rnn_output, self.llogits]
