...
def getdescriptionsembedding(entid):
    ...
    try:
        ...
        r = requests.post("http://localhost:8887/ftwv_dec",json={'chunks': [description]},headers={'Connection':'close'})
...
def CreateVectors(inputcandidatetuple):
    ...
    try:
        ...
        word = chunks[idx][0]
        res = es.search(index="wikidataentitylabelindex03", body={"query":{"multi_match":{"query":chunks[idx][0]}},"size":30})
        ...
        if idx > 0:
             ...
             res = es.search(index="wikidataentitylabelindex03", body={"query":{"multi_match":{"query":word}},"size":30})
             esresults = res['hits']['hits']
             if len(esresults) > 0:
                 for entidx,esresult in enumerate(esresults):
                     descembedding = getdescriptionsembedding(esresult['_source']['uri'][37:])
...
class Vectoriser():
    ...
    def vectorise(self,nlquery):
        ...
        r = requests.post("http://localhost:8887/ftwv_tok",json={'chunks': tokens},headers={'Connection':'close'})
